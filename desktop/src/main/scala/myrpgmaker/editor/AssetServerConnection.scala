...
import myrpgmaker.editor.dialog.SettingsDialog

import myrpgmaker.editor.util.PackageManager
...
class AssetServerConnection(val mainP: MainPanel,sm: StateMaster) {
  ...
  def ready() = {
    ...
    var password = Settings.get("assetserver.password").get
		...
		var fullHost = host
		if(host=="") {
			host = "assets.myrpgmaker.com"
			fullHost = "http://assets.myrpgmaker.com"
		} else {
