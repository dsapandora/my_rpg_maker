...
object RpgDesktop
  ...
  override def startup(args: Array[String]) = {
    ...
    val icon = myrpgmaker.lib.Utils.readClasspathImage("icon.png")

    System.setProperty("sun.awt.exception.handler", "myrpgmaker.editor.EDTErrorHandler");
  ...
  override def main(args: Array[String]) {
    if (args.size >= 2 && args.head == "--player") {
      myrpgmaker.player.LwjglPlayer.main(args.tail)
    } else {
      super.main(args)
    }
  }
